...
// // script.js - كامل للواجهة (مضمن معه دوال الرفع إلى code.gs والتحميل المنقّح للقوائم)
// // قبل الاستخدام: ضع رابط Web App في API_URL
// const API_URL = 'REPLACE_WITH_YOUR_WEBAPP_URL'; // مثال: 'https://script.google.com/macros/s/XXXX/exec'

// /* =========== Helpers: apiFetch / apiPost =========== */
// async function apiFetch(url, opts = {}) {
//   try {
//     const res = await fetch(url, opts);
//     const text = await res.text();
...
// /* =========== loadLookupsAndPopulate (محسّن) =========== */
// async function loadLookupsAndPopulate() {
//   try {
//     const resp = await apiFetch(`${API_URL}?action=getLookups`);
//     if (!resp.ok) { console.warn('getLookups failed', resp); showError('تعذر تحميل القوائم من الخادم'); return; }
//     const json = resp.data;
//     let payload = null;
//     if (json && typeof json === 'object') {
//       if (json.success && json.data) payload = json.data;
//       else payload = json;
//       if (payload.data && typeof payload.data === 'object' && (payload.data.packages || payload.data.cities)) payload = payload.data;
//     }
//     if (!payload) { console.warn('loadLookupsAndPopulate: no payload'); return; }

//     const pkgs = payload.packages || payload.packagesList || payload.packagesArray || [];
//     const cities = payload.cities || [];
//     const areas = payload.areas || [];
//     const sites = payload.sites || [];
//     const activities = payload.activities || [];
//     const paymentsMethods = payload.paymentsMethods || payload.payments || [];
...
//     const cityAreaMap = {};
//     (areas || []).forEach(a => {
//       const cid = (a.raw && (a.raw['ID المدينة'] || a.raw['cityId'] || a.raw['city'])) ? String(a.raw['ID المدينة'] || a.raw['cityId'] || a.raw['city']) : String(a.id || '');
//       if (!cityAreaMap[cid]) cityAreaMap[cid] = [];
//       cityAreaMap[cid].push({ id: a.id, name: a.name, raw: a.raw });
//     });
//     window.cityAreaMap = cityAreaMap;
...
//     // packages select & grid
//     const pkgSelect = document.querySelector('select[name="package"]');
//     if (pkgSelect) {
//       pkgSelect.innerHTML = '<option value="">اختر الباقة</option>';
//       (pkgs || []).forEach(p => {
//         const label = (p.name || (p.raw && (p.raw['اسم الباقة'] || 'بدون اسم'))) + (p.duration ? ` (${p.duration} يوم)` : '');
//         const opt = document.createElement('option'); opt.value = p.id || (p.raw && (p.raw['ID الباقة'] || p.raw['id']) ) || ''; opt.textContent = label; pkgSelect.appendChild(opt);
//       });
//     }
...
//       (pkgs || []).forEach(p => {
//         const div = document.createElement('div'); div.className = 'pkg-card';
//         const h = document.createElement('h3'); h.textContent = p.name || (p.raw && (p.raw['اسم الباقة'] || 'بدون اسم'));
//         const d = document.createElement('p'); d.textContent = `المدة: ${p.duration || (p.raw && (p.raw['مدة الباقة باليوم'] || ''))} يوم`;
//         const price = document.createElement('p'); price.textContent = `السعر: ${p.price || (p.raw && (p.raw['سعر الباقة'] || p.raw['السعر'] || ''))}`;
//         const btn = document.createElement('button'); btn.className = 'choose-pkg'; btn.textContent = 'اختر الباقة';
//         btn.onclick = () => choosePackageAPI(p.id || (p.raw && (p.raw['ID الباقة'] || '')));
//         div.appendChild(h); div.appendChild(d); div.appendChild(price);
//         if (p.raw && p.raw['وصف الباقة']) { const desc = document.createElement('p'); desc.textContent = p.raw['وصف الباقة']; div.appendChild(desc); }
//         div.appendChild(btn);
//         pkgGrid.appendChild(div);
//       });
//     }

//     window.availablePaymentMethods = (paymentsMethods || []).map(pm => ({ id: pm.id || (pm.raw && pm.raw['معرف الدفع']), name: pm.name || (pm.raw && (pm.raw['طرق الدفع'] || pm.raw['طريقة الدفع'])), raw: pm.raw || pm }));
...
//   } catch (err) {
//     console.error('loadLookupsAndPopulate error', err);
//     showError('تعذر تحميل القوائم (راجع Console)');
//   }
// }

// /* =========== UI helpers (success/error/loading) =========== */
// function showSuccess(message) { const el = document.getElementById('successAlert'); if (!el) return; el.textContent = message; el.className = 'alert alert-success'; el.style.display = 'block'; setTimeout(()=>el.style.display='none',5000); }
// function showError(message) { const el = document.getElementById('errorAlert'); if (!el) return; el.textContent = message; el.className = 'alert alert-error'; el.style.display = 'block'; setTimeout(()=>el.style.display='none',6000); }
// function showLoading(show) { const el = document.getElementById('loading'); if (!el) return; el.style.display = show ? 'block' : 'none'; }

// /* =========== Simple integration points used by the UI =========== */
// // getLoggedPlace / setLoggedPlace used for session on client
// function getLoggedPlace() { try { const raw = localStorage.getItem('khedmatak_place'); return raw ? JSON.parse(raw) : null; } catch (e) { return null; } }
// function setLoggedPlace(obj) { try { localStorage.setItem('khedmatak_place', JSON.stringify(obj)); } catch (e) {} }
// function clearLoggedPlace() { localStorage.removeItem('khedmatak_place'); }

// /* =========== Minimal initializers =========== */
// document.addEventListener('DOMContentLoaded', () => {
//   loadLookupsAndPopulate().catch(()=>{});
//   loadPlacesForAds().catch(()=>{});
// });

// /* =========== loadPlacesForAds used by UI selects =========== */
// async function loadPlacesForAds() {
//   const placeSelects = document.querySelectorAll('select[name="placeId"]');
//   placeSelects.forEach(ps => { ps.innerHTML = '<option value="">اختر المكان</option>'; });
//   const resp = await apiFetch(`${API_URL}?action=places`);
//   if (!resp.ok) { console.warn('loadPlacesForAds: failed', resp); return; }
//   const json = resp.data;
//   let places = [];
//   if (json && json.success && json.data && Array.isArray(json.data.places)) places = json.data.places;
//   else if (json && Array.isArray(json.places)) places = json.places;
...
async function apiFetch(url, opts = {}) {
  try {
    ...
    catch (e) { return { ok: res.ok, status: res.status, data: text, raw: text }; }
  } catch (err) {
    return { ok: false, status: 0, error: err.message || String(err) };
  }
}
...
// =================== UI helpers ===================
function showSuccess(msg) { const el = document.getElementById('successAlert'); if (!el) return; el.textContent = msg; el.className = 'alert alert-success'; el.style.display = 'block'; setTimeout(()=>el.style.display='none',5000); }
function showError(msg) { const el = document.getElementById('errorAlert'); if (!el) return; el.textContent = msg; el.className = 'alert alert-error'; el.style.display = 'block'; setTimeout(()=>el.style.display='none',6000); }
function showLoading(on) { const el = document.getElementById('loading'); if (!el) return; el.style.display = on ? 'block' : 'none'; }
...
// uploadMediaClient: read file -> call server action=uploadMedia (imgbb optional)
// returns { success, fileUrl, imgbbUrl, raw, error }
async function uploadMediaClient(file, placeId = '', imgbb = true) {
  try {
    const base64 = await readFileAsBase64(file);
    const payload = {
    ...
    const resp = await apiPost(payload);
    if (!resp.ok) throw new Error((resp.data && resp.data.error) ? resp.data.error : 'upload failed');
    const d = resp.data && resp.data.data ? resp.data.data : resp.data;
    return { success: true, fileUrl: (d && d.fileUrl) || '', imgbbUrl: (d && d.imgbbUrl) || '', raw: d || resp.data };
  } catch (err) {
    console.error('uploadMediaClient', err);
    return { success: false, error: err.message || String(err) };
  }
}

// Convenience: uploadLogo(file, placeId) -> writes both Drive and imgbb links server-side (if supported)
async function uploadLogo(file, placeId) {
  if (!file) throw new Error('No file provided');
  const r = await uploadMediaClient(file, placeId, true);
  if (!r.success) throw new Error(r.error || 'upload failed');
...
// =================== Lookups loading & populate ===================
async function loadLookupsAndPopulate() {
  try {
    const resp = await apiFetch(`${API_URL}?action=getLookups`);
    if (!resp.ok) { console.warn('getLookups failed', resp); showError('تعذر تحميل القوائم'); return; }
    const json = resp.data;
    // normalize payload shapes
    let payload = null;
    if (json && typeof json === 'object') {
      payload = (json.success && json.data) ? json.data : json;
      if (payload.data && typeof payload.data === 'object' && (payload.data.packages || payload.data.cities)) payload = payload.data;
    }
    if (!payload) { console.warn('no payload from getLookups'); return; }
    ...
    const sites = payload.sites || [];
    const activities = payload.activities || [];
    const payments = payload.paymentsMethods || payload.payments || [];

    // activities
    const actSel = document.querySelector('select[name="activityType"]');
    if (actSel) {
      actSel.innerHTML = '<option value="">اختر نوع النشاط</option>';
      activities.forEach(a => { const opt = document.createElement('option'); opt.value = a.id; opt.textContent = a.name; actSel.appendChild(opt); });
    }
    ...
    siteSelects.forEach(s => {
      s.innerHTML = '<option value="">اختر الموقع</option>';
      sites.forEach(site => { const opt = document.createElement('option'); opt.value = site.id; opt.textContent = site.name; s.appendChild(opt); });
    });

    // packages select and grid
    const pkgSelect = document.querySelector('select[name="package"]');
    if (pkgSelect) {
      pkgSelect.innerHTML = '<option value="">اختر الباقة</option>';
      pkgs.forEach(p => {
        const label = (p.name || (p.raw && (p.raw['اسم الباقة'] || '')) ) + (p.duration ? ` (${p.duration} يوم)` : '');
        const opt = document.createElement('option'); opt.value = p.id || (p.raw && (p.raw['ID الباقة'] || p.raw.id)) || ''; opt.textContent = label;
    ...
    // store payment methods for payment UI if needed
    window.availablePaymentMethods = (payments || []).map(pm => ({ id: pm.id || (pm.raw && pm.raw['معرف الدفع']), name: pm.name || (pm.raw && (pm.raw['طرق الدفع'] || pm.raw['طريقة الدفع'])), raw: pm.raw || pm }));

  } catch (err) {
    console.error('loadLookupsAndPopulate', err);
    showError('تعذر تحميل القوائم (انظر Console)');
  }
}

// =================== Places select loader ===================
async function loadPlacesForAds() {
  const placeSelects = document.querySelectorAll('select[name="placeId"]');
  placeSelects.forEach(ps => ps.innerHTML = '<option value="">اختر المكان</option>');
  const resp = await apiFetch(`${API_URL}?action=places`);
  ...
  if (logged && logged.id) {
    placeSelects.forEach(ps => { ps.value = logged.id; ps.disabled = true; });
    if (typeof loadAdsForPlace === 'function') loadAdsForPlace(logged.id);
  } else placeSelects.forEach(ps => ps.disabled = false);
}
...
function previewImage(inputEl, previewId) {
  ...
  if (inputEl.files && inputEl.files[0]) {
    ...
    const r = new FileReader();
    r.onload = e => { const img = document.createElement('img'); img.src = e.target.result; img.style.maxWidth='120px'; img.style.borderRadius='6px'; preview.appendChild(img); uploadedImages = [f]; };
    r.readAsDataURL(f);
  }
}

function previewMultipleImages(inputEl, previewId) {
  const preview = document.getElementById(previewId); if (!preview) return;
  preview.innerHTML = ''; uploadedImages = [];
  ...
  files.forEach(file => {
    ...
    r.onload = e => {
      const div = document.createElement('div'); div.className='preview-image';
      const img = document.createElement('img'); img.src = e.target.result; img.style.width='100px'; img.style.height='80px'; img.style.objectFit='cover'; img.style.borderRadius='6px';
      const btn = document.createElement('button'); btn.textContent='×'; btn.type='button'; btn.className='remove-image';
      btn.onclick = () => { div.remove(); uploadedImages = uploadedImages.filter(x=>x!==file); };
...
function previewVideo(inputEl, previewId) {
  const preview = document.getElementById(previewId); if (!preview) return;
  preview.innerHTML = ''; uploadedVideos = [];
  if (inputEl.files && inputEl.files[0]) {
    const file = inputEl.files[0];
    const r = new FileReader();
    r.onload = e => { const vid = document.createElement('video'); vid.src = e.target.result; vid.controls = true; vid.style.width='100%'; preview.appendChild(vid); uploadedVideos = [file]; };
    r.readAsDataURL(file);
  }
}

// =================== Place form submit handler ===================
async function handlePlaceSubmit(ev) {
  ev.preventDefault();
  showLoading(true);
  try {
    const form = ev.target;
    const fd = new FormData(form);
    const fileInput = form.querySelector('input[name="placeImage"]');
    let driveUrl = '', imgbbUrl = '';
    const logged = getLoggedPlace();

    if (fileInput && fileInput.files && fileInput.files[0]) {
      // upload and ask server to also upload to imgbb so we get public link
      const up = await uploadMediaClient(fileInput.files[0], logged && logged.id ? logged.id : '', true);
      if (!up.success) {
        // fallback: attempt drive-only
        showError('تعذر رفع الشعار إلى imgbb; تم رفعه فقط إلى Drive');
        driveUrl = up.fileUrl || '';
      } else {
        driveUrl = up.fileUrl || '';
        imgbbUrl = up.imgbbUrl || '';
    ...
    // map form fields to server keys
    if (fd.get('placeName')) payload.name = fd.get('placeName');
    if (fd.get('activityType')) payload.activityId = fd.get('activityType');
    if (fd.get('city')) payload.cityId = fd.get('city');
    if (fd.get('area')) payload.areaId = fd.get('area');
    if (fd.get('location')) payload.siteId = fd.get('location');
    if (fd.get('detailedAddress')) payload.address = fd.get('detailedAddress');
    if (fd.get('mapLink')) payload.mapLink = fd.get('mapLink');
    if (fd.get('phone')) payload.phone = fd.get('phone');
    if (fd.get('whatsappLink')) payload.whatsapp = fd.get('whatsappLink');
    if (fd.get('email')) payload.email = fd.get('email');
    if (fd.get('website')) payload.website = fd.get('website');
    if (fd.get('workingHours')) payload.hours = fd.get('workingHours');
    if (fd.get('delivery')) payload.delivery = fd.get('delivery');
    if (fd.get('package')) payload.packageId = fd.get('package');
    if (fd.get('description')) payload.description = fd.get('description');
    if (fd.get('password')) payload.password = fd.get('password');
    if (fd.get('status')) payload.status = fd.get('status');
    if (driveUrl) payload.logoUrl = driveUrl;
    if (imgbbUrl) payload.imgbbLogoUrl = imgbbUrl;

    const resp = await apiPost(payload);
    if (!resp.ok) throw new Error((resp.data && resp.data.error) ? resp.data.error : 'فشل حفظ المكان');
    const data = resp.data;
    if (data && data.success === false) throw new Error(data.error || 'فشل في الاستجابة من الخادم');

    // try to update client session with returned place
    if (data.data && data.data.place) { setLoggedPlace(data.data.place); setLoggedInUI(data.data.place); }
    else if (data.data && data.data.id) {
      // fetch dashboard to get full place
      const fetched = await fetchPlace(data.data.id);
      if (fetched) { setLoggedPlace(fetched); setLoggedInUI(fetched); }
    }
    ...
    console.error('handlePlaceSubmit', err);
    showError(err.message || 'خطأ أثناء حفظ المكان');
  } finally { showLoading(false); }
}
...
async function handleAdSubmit(ev) {
  ...
  try {
    const form = ev.target;
    const fd = new FormData(form);
    const placeId = fd.get('placeId') || (getLoggedPlace() && getLoggedPlace().id) || '';
    if (!placeId) { showError('اختر مكانًا للإعلان'); showLoading(false); return; }

    // collect image base64s (preferred server-side upload)
    const imageBase64s = [];
    const imgFiles = (uploadedImages && uploadedImages.length) ? uploadedImages : [];
    for (let i = 0; i < Math.min(imgFiles.length, 8); i++) {
      const b = await readFileAsBase64(imgFiles[i]);
      imageBase64s.push(b);
    }

    // video (optional) - we don't upload video to imgbb
    let videoFileName = '';
    if (uploadedVideos && uploadedVideos[0]) videoFileName = uploadedVideos[0].name || '';
    ...
    const payload = {
      ...
      endDate: fd.get('endDate') || '',
      coupon: fd.get('coupon') || '',
      imageBase64s: imageBase64s, // server will upload to Drive + imgbb and fill columns
      videoFile: videoFileName,
      videoUrl: '' // leave empty or pass a hosted URL if available
    };

    const resp = await apiPost(payload);
    if (!resp.ok) throw new Error((resp.data && resp.data.error) ? resp.data.error : 'فشل حفظ الإعلان');
    const data = resp.data;
    if (data && data.success === false) throw new Error(data.error || 'فشل في الاستجابة من الخادم');

    showSuccess('تم إضافة الإعلان');
    form.reset();
    const preview = document.getElementById('adImagesPreview'); if (preview) preview.innerHTML = '';
    uploadedImages = []; uploadedVideos = [];
    if (typeof loadAdsForPlace === 'function') loadAdsForPlace(placeId);
  } catch (err) {
    console.error('handleAdSubmit', err);
    showError(err.message || 'خطأ أثناء حفظ الإعلان');
  } finally { showLoading(false); }
}

// =================== Fetch place (dashboard) ===================
async function fetchPlace(placeId) {
...
async function handleLogin(formEl) {
  try {
    const phoneOrId = formEl.querySelector('input[name="phoneOrId"]').value.trim();
    const password = formEl.querySelector('input[name="password"]').value || '';
    if (!phoneOrId || !password) { showError('ادخل بيانات الدخول'); return; }
    showLoading(true);
    const resp = await apiPost({ action: 'loginPlace', phoneOrId, password });
    if (!resp.ok) { showError('فشل التواصل مع الخادم'); return; }
    const data = resp.data;
    if (!data || data.success === false) { showError(data && data.error ? data.error : 'فشل الدخول'); return; }
...
async function setLoggedInUI(place) {
  ...
  if (nameEl) nameEl.style.display = 'inline-block', nameEl.textContent = place && place.name ? place.name : 'صاحب المحل';
  setLoggedPlace(place);
  await loadPlacesForAds();
}
...
async function choosePackageAPI(packageId) {
  const logged = getLoggedPlace();
  if (!logged || !logged.id) return showError('يجب تسجيل الدخول لاختيار الباقة');
  showLoading(true);
  try {
    const resp = await apiPost({ action: 'choosePackage', placeId: logged.id, packageId: packageId });
    if (!resp.ok) throw new Error('فشل التواصل مع الخادم');
    const data = resp.data;
    if (!data || data.success === false) throw new Error((data && data.error) ? data.error : 'خطأ غير متوقع');
    const d = data.data || data;
    if (d && d.pending) {
      // open payment modal to upload receipt
      showPaymentModal({ paymentId: d.paymentId, amount: d.amount, currency: d.currency || 'SAR', placeId: logged.id });
      showSuccess('تم إنشاء طلب دفع — أرسل إيصال الدفع لإتمام التفعيل');
    } else {
    ...
    console.error('choosePackageAPI', err);
    showError(err.message || 'فشل اختيار الباقة');
  } finally { showLoading(false); }
}

// =================== Payment modal & upload receipt ===================
function showPaymentModal({ paymentId, amount, currency, placeId }) {
  // minimal modal implementation expecting DOM elements exist
  const modal = document.getElementById('paymentModal');
  if (!modal) {
    alert(`أرسل إيصال الدفع يدوياً: paymentId=${paymentId}، amount=${amount} ${currency}`);
    return;
  }
  modal.style.display = 'flex';
  modal.querySelector('.payment-id').textContent = paymentId || '';
  modal.querySelector('.payment-amount').textContent = `${amount || ''} ${currency || ''}`;
  const fileInput = modal.querySelector('input[name="paymentReceipt"]');
  const submitBtn = modal.querySelector('button[name="sendReceipt"]');
  if (submitBtn) {
    submitBtn.onclick = async () => {
      if (!fileInput || !fileInput.files || !fileInput.files[0]) return showError('اختر ملف الإيصال');
      showLoading(true);
      try {
        const up = await uploadMediaClient(fileInput.files[0], placeId, true);
        if (!up.success) throw new Error(up.error || 'فشل رفع الإيصال');
        // update payment request: set canonical receipt link and status
        const updates = { 'رابط إيصال الدفع': up.imgbbUrl || up.fileUrl || '', 'الحالة': 'قيد المراجعة' };
        const resp = await apiPost({ action: 'updatePaymentRequest', paymentId: paymentId, updates: updates });
        if (!resp.ok) throw new Error('فشل حفظ بيانات الإيصال');
        modal.style.display = 'none';
        showSuccess('تم إرسال الإيصال بنجاح، في انتظار التفعيل');
      } catch (err) {
        console.error('send receipt', err);
        showError(err.message || 'خطأ أثناء إرسال الإيصال');
      } finally { showLoading(false); }
    };
  }
...
// =================== Init bindings (minimal) ===================
document.addEventListener('DOMContentLoaded', () => {
  // wire up file previews if elements exist
  const placeImageInput = document.querySelector('input[name="placeImage"]');
  if (placeImageInput) placeImageInput.addEventListener('change', (e)=>previewImage(e.target,'placeImagePreview'));

  const adImagesInput = document.querySelector('input[name="adImages"]');
  if (adImagesInput) adImagesInput.addEventListener('change', (e)=>previewMultipleImages(e.target,'adImagesPreview'));

  const adVideoInput = document.querySelector('input[name="adVideo"]');
  if (adVideoInput) adVideoInput.addEventListener('change', (e)=>previewVideo(e.target,'adVideoPreview'));
  ...
  const loginForm = document.getElementById('loginForm');
  if (loginForm) loginForm.addEventListener('submit', (ev) => { ev.preventDefault(); handleLogin(loginForm); });
  ...
  // if logged in, set UI
  const stored = getLoggedPlace();
  if (stored && stored.id) setLoggedInUI(stored);
});
