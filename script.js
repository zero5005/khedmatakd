...
// merged script.js — includes payment UI (modal + upload receipt) integrated with choosePackage flow
...
async function choosePackageAPI(packageId) {
  ...
  try {
    ...
    // Server returns either pending:true (paid package -> paymentId) or pending:false for free packages
    const d = (data && data.data) ? data.data : data;
    if (d && d.pending) {
      // paid package — show payment modal so owner can pay/upload receipt
      ...
      // update local place state: show 'قيد الدفع' in UI
      const place = getLoggedPlace() || {};
      place.raw = place.raw || {};
      ...
      // free package activated immediately or server handled activation
      showSuccess('تم تغيير الباقة');
      if (d && d.start && d.end) {
        const place = getLoggedPlace() || {};
...
/*
  showPaymentModal(params):
    params = { paymentId, amount, currency, placeId }
  Requires:
    - readFileAsBase64(file)
    - API endpoints:
      - uploadMedia (action: 'uploadMedia') returns { success:true, data:{fileUrl: '...'} } or { fileUrl: '...' }
      - updatePaymentRequest (action: 'updatePaymentRequest') to set receiptUrl/status on payment row
    If updatePaymentRequest is not implemented server-side, admin can still verify manually using Drive link.
*/
function showPaymentModal({ paymentId, amount, currency, placeId }) {
  ...
  modal.innerHTML = `
    <div style="background:#fff;padding:18px;border-radius:10px;max-width:720px;width:95%;direction:rtl;color:#111">
      ...
      <p>معرف طلب الدفع: <strong id="pm_paymentId">${paymentId}</strong></p>
  ...
  // render payment methods (from cache window.availablePaymentMethods or fetch lookups)
  ...
  btnSend.addEventListener('click', async () => {
    ...
    try {
      ...
      // upload via uploadMedia endpoint
      const uploadPayload = {
        action: 'uploadMedia',
        fileName: file.name,
        mimeType: file.type,
      ...
      // server may return fileUrl in different places; normalize
      ...
      // update payment request row with receipt link (requires updatePaymentRequest endpoint in code.gs)
      const updatePayload = {
        action: 'updatePaymentRequest',
        paymentId: paymentId,
        updates: { 'رابط إيصال الدفع': fileUrl, 'receiptUrl': fileUrl, status: 'receipt_uploaded', notes: 'تم رفع إيصال من صاحب المحل' }
      };
      const updateResp = await apiPost(updatePayload);
      if (!updateResp.ok) {
        // not fatal: still inform user and keep fileUrl (admin can check Drive)
...
/* ================= Remaining helpers and code.gs interactions ================= */
// The rest of helper functions like fetchPlace, uploadMedia server interaction are used above.
// Make sure your code.gs implements 'uploadMedia' and 'updatePaymentRequest' actions as used by the payment UI.
